...
function RoomModal({ showModal, setShowModal, setRoomArray }) {
	...
	async function createRoom() {
		try {
			const res = await fetch("http://localhost:5000/api/create-room", {
				...
				// update roomarray so when user creates room it shows under create room button without them having to click refresh
			});

			// make to modal
			//			setRoomArray(prev => [...prev, data.data]);
